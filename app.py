...
def predict():
    ...
    if 'file' in request.files:
        ...
        if file.filename != '':
            ...
            predicted_class = ' '.join(word.capitalize() for word in predicted_class.replace('__', ' ').replace('_', ' ').split(' ')).replace('  ', ' ')
    ...
    return render_template('result.html', image_path=image_path, predicted_class=predicted_class, prediction_confidence=confidence, insights=insights)
